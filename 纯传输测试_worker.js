...
let 咦这是我的私钥哎 = "123456"; //去掉了UUID验证，改用路径私钥强制验证，路径添加my-key=

let 启用反代功能 = true //选择是否启用反代功能【总开关】，false，true，现在你可以自由的选择是否启用反代功能了
let 反代IP = '24.199.74.252' //反代IP或域名，反代IP端口一般情况下不用填写，如果你非要用非标反代的话，可以填'ts.hpc.tw:443'这样
...
let 本地严格缓存 = true //新增了脚本本地缓存，可以维持几分钟甚至几小时左右，如果启用则严格按照TTL时间进行缓存，关闭可减少频繁查询的次数，提升速度，个人视使用环境选择
...
export default {
  async fetch(访问请求) {
    ...
    const 提取私钥 = 读取路径.match(/my-key=([^&]+)/)?.[1];
    if (访问请求.headers.get('Upgrade') === 'websocket' && 提取私钥 === 咦这是我的私钥哎){
...
async function 解析VL标头(WS接口, TCP接口) {
  let 识别地址类型, 访问地址, 地址长度, 开始传输时间 = performance.now();
  const 二进制数据 = await new Promise(resolve => { WS接口.addEventListener('message', event => { resolve(new Uint8Array(event.data)); }, { once: true }); });
  try {
    ...
    if (访问端口 === 53) { //这个处理是应对某些客户端优先强制查询dns的情况，也是通过加密通道udp over tcp的，放心使用
      ...
      WS接口.send(await new Blob([构建长度头部, 提取DOH结果, new TextEncoder().encode(生成随机字符串(500, 1000))]).arrayBuffer());
      return;
    }
    ...
    switch (识别地址类型) {
      ...
      case 2:
        ...
        if (启用DOH查询转换) {
          ...
          if (匹配地址(访问地址)) 识别地址类型 = 访问地址.includes(':') ? 3 : 1;
        } else {
          访问地址 = 访问域名;
        }
        break;
    ...
    if (启用新版传输模式) {
      ...
      队列传输管道(开始传输时间, 访问地址, 传输数据, 读取数据, WS接口); //建立WS接口与TCP接口的传输管道
    } else {
...
async function 队列传输管道(开始传输时间, 访问地址, 传输数据, 读取数据, WS接口, 传输队列 = Promise.resolve(), 字节计数 = 0, 累计传输字节数 = 0, 大数据包 = false, 已结束 = false) {
  WS接口.addEventListener('message', event => {
    ...
    while (分段初值 < WS数据.length) {
      ...
      传输队列 = 传输队列.then(async () => {try { await 传输数据.write(数据块) } catch {}}).catch();
      分段初值 += 分段大小;
    }
    累计传输字节数 += WS数据.length;
  });
