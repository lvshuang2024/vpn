...
const S = 'cmy51488', // 订阅路径，随便改
      U = '63d60ca2-c97f-44c1-a000-10af1d5406f5', // 你自己的 UUID
      R = 'sjc.o00o.ooo',
      N = 'CFKB',
      B = [90,186,91,119,72,235,74,226,182,13,91,254,231,172,22,158],
      K = new Uint8Array(2),
      P = ['104.16.160.145'],
      H = new Headers({'cache-control':'public,max-age=14400','content-type':'text/plain'}),
      d = new TextDecoder(),
      M = new Map();
...
const tryConn = async(opts,ms=3000)=>{
  try{const s=C(opts); return await Promise.race([s.opened.then(()=>s),new Promise((_,r)=>setTimeout(()=>r(new Error('timeout')),ms))]);}
  catch(e){throw e;}
};

export default {
 async fetch(r){
  const url=r.url,sl=url.indexOf('//')+2,hEnd=url.indexOf('/',sl),
        host=hEnd===-1?url.slice(sl):url.slice(sl,hEnd),
        path=hEnd===-1?'/':url.slice(hEnd);
  ...
  if(r.headers.get('Upgrade')!=='websocket'){
   ...
   if(path===`/${S}/vless`){
    ...
    const q=`host=${host}&sni=${host}&path=%2F%3Fed%3D2560#${N}`,
          f=i=>`vless://${U}@${i}:443?encryption=none&security=tls&type=ws&${q}`;
  ...
  const pr=r.headers.get('sec-websocket-protocol');
  if(!pr) return new Response(null,{status:400});

  // === 改动点：跳过 UUID 签名校验 ===
  // const p1=decodePrefix(pr,18);
  // if(!checkSig(p1)) return new Response(null,{status:403});
  // ==================================
  ...
  let s;
  try{s=await tryConn({hostname:addr,port,secureTransport:'off'});}
  catch{try{s=await tryConn({hostname:R,port:443,secureTransport:'off'});}catch{return new Response(null,{status:502});}}
